    ...
    def put(self, request):
        ...
        if error is None:
            ...
            if not req:
                return json_response(error='未找到指定发布申请')
            if form.action == 'check':
                return json_response({'date':req.created_at,'name':req.name})
            DeployRequest.objects.create(
                deploy_id=req.deploy_id,
                name=f'{req.name} - 回滚',
                type='2',
                extra=req.extra,
                host_ids=req.host_ids,
                status='0' if req.deploy.is_audit else '1',
                desc='自动回滚至该应用的上个版本',
                version=req.version,
                created_by=request.user
            )
...
class RequestDetailView(View):
    def get(self, request, r_id):
        ...
        targets = [{
            'id': x.id,
            'title': f'{x.name}({x.hostname}:{x.port})'
        } for x in hosts]
        ...
        if request.GET.get('log'):
            rds, key, counter = get_redis_connection(
            ), f'{settings.REQUEST_KEY}:{r_id}', 0
